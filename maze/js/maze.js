...
class Maze {
    ...
    drawCorrectPath(x, y) {
		...
		var preX = maze.mazeGrids[y][x].preGrid.x,
            preY = maze.mazeGrids[y][x].preGrid.y;
		t=setTimeout(function(){then()},50);
		function then() {     
		maze.drawCorrectPath(preX, preY);
	}
    }
    ...
    moveBall(x, y, useAcc) {
        ...
        var bx = this.ballX,
            by = this.ballY;
		/*this.fillGrid(bx/this.step, by/this.step, "#0066FF");*/
        if (useAcc) {
            // 应用重力加速度：
            // 每秒速度增加量等于重力加速度；
